/*his program reads a random access file sequentially, updates data already written to the file, creates new data to be placed in the file, and
 deletes data previously in the file.*/
 #include <stdio.h>
...
void textFile( FILE *readPtr ){
  ...
  if ( ( writePtr = fopen( "accounts.txt", "w" ) ) == NULL ) {
    ...
    //y all records from random-access file into text file
    while ( !feof( readPtr ) ) {
...
void newRecord( FILE *fPtr ){
  ...
  if ( client.acctNum != 0 ) {
    ...
    //ser enters last name, first name and balance
